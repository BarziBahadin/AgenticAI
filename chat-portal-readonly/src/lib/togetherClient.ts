...
export type { AuditInput, AuditOutput, GenerateAuditOptions } from "./geminiClient";
...
// Resource management for cloud calls
...
async function withResourceLimit<T>(fn: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if (REQUEST_QUEUE.length > 0 && activeRequests < MAX_CONCURRENT_REQUESTS) {
